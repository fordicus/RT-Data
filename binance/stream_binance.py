...
async def orderbook_writer():
    while True:
        ...
        filename = f"{symbol.upper()}_orderbook_{date_str}.jsonl"
        file_path = os.path.join(LOB_DIR, filename)

        last_opened, writer = file_handles.get(symbol, (None, None))

        if last_opened != date_str:
            if writer:
                writer.close()
                await writer.wait_closed()
            writer = await asyncio.to_thread(open, file_path, "a", encoding="utf-8")
            file_handles[symbol] = (date_str, writer)
