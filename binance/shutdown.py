...
Provides centralized shutdown handling with proper resource cleanup,
avoiding global state and race conditions.
"""
...
class ShutdownManager:
	"""
	Centralized shutdown manager that handles graceful application termination.
	...
	def register_executors(self, **executors: ProcessPoolExecutor) -> None:
		"""
	...
	def register_file_handles(self, file_handles: Dict[str, Tuple[str, TextIOWrapper]]) -> None:
		"""
	...
	def shutdown_executors(self) -> None:
		"""
		Gracefully shutdown all registered executors with individual logging.
		Thread-safe and prevents duplicate shutdown attempts.
		"""
		...
		for name, executor in executors_copy.items():
			try:
				if executor:
					self.logger.info(f"[ShutdownManager] Shutting down {name.upper()}_EXECUTOR...")
	...
	def run_custom_cleanup(self) -> None:
		...
		for callback in self._custom_cleanup_callbacks:
			try:
				...
				self.logger.error(
					f"[ShutdownManager] Custom cleanup callback failed: {e}",
					exc_info=True
				)
	...
	def graceful_shutdown(self) -> None:
		...
		try:
			self.logger.info("[ShutdownManager] Starting graceful shutdown...")
			...
			self.logger.info("[ShutdownManager] Graceful shutdown completed.")
			
		except Exception as e:
			self.logger.error(f"[ShutdownManager] Error during shutdown: {e}")
